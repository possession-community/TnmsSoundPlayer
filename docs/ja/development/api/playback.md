...
| スコアボードからの個別音量調整 | `SpeakerSteamId` の設定が必要 | できない |

**ボットモード**は音声をプレイヤースロットに紐付けるので、そのスロットを誰かで埋めるためにボットを 1 体置く必要がある。
つまりボットモードはサーバーの 64 スロットのうち 1 つを常時占有する。
...
プレイヤー自身にスピーカーの音量を調整させたい場合は、この値を設定すること。
スコアボードの個別音量調整は実プレイヤーの行に属するもので、ボット扱いのままの行には出ない。
設定しない場合、音量を変える手段はサーバー側の `SetPlayerVolume` だけになる。
