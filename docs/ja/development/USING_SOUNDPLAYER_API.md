...
`tnms_sound_speaker_mode 0` にすると、音声はモジュールが観戦席に置いているボットの声として送出される。
...
プレイヤー自身にスコアボードからスピーカーの音量を調整させたい場合も、この設定が必要になる。
スコアボードの音量調整は実プレイヤーの行に属するもので、ボット扱いのままの行には出ないためである。
...
既定（`1`）では、音声はエンティティインデックスに紐付けられる。
